...
const ActionSheet = ({
  ...
  useEffect(() => {
    ...
    const ActionSheet = document.querySelector(
      `.${style.ActionSheet}.${style.active}`
    );
    const ActionBox = document.querySelector(
      `.${style.ActionBox}.${style.active}`
    );
  ...
  const deleteLog = async () => {
    console.log('삭제 클릭');
    const response = await fetch(`${url}/codi/codiDelete/${codiLogId}`, {
      method: 'DELETE',
    });
    if (response.ok) {
      console.log('삭제 완료');
  ...
  return (
    <section
      className={`${style.ActionSheet} ${
        actionSheetActive ? style.active : ''
      }`}
    >
      <div
      ...
      >
        {canEdit ? (
          <button className="fontTitleM" onClick={editLog}>
            수정하기
          </button>
        ) : (
          ''
        )}
        <button className="fontTitleM" onClick={() => setModalActive(true)}>
          삭제하기
        </button>
      </div>

      {modalActive ? (
        <div className={style.modalCheck}>
          <div className={style.modalBox}>
            <span className="fontHead3">코디 기록을 삭제하시겠습니까?</span>
            <div className={style.btns}>
              <button
                className={`fontTitleM ${style.btnCancel}`}
