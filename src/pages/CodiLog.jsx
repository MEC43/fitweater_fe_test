...
const CodiLog = () => {
  ...
  const fetchLog = (page, reset = false) => {
    ...
    try {
      fetch(
        `${url}/codi/codiLogList/${userInfo.userid}?page=${page}&limit=${limit}`
      )
        .then((res) => res.json()) //
        .then((data) => {
          if (reset) {
            // reset 파라미터가 true이면 데이터 초기화
  ...
  useEffect(() => {
    //선택한 태그를 포함하는 게시물 필터링
    if (feltWeather.length) {
      //필터링 선택한게 있다면..
      const filteredList = ALLcodiLogList.filter((codiLog) => {
        return feltWeather.every((activeTag) =>
          codiLog.tag.includes(activeTag)
        ); //every() -> feltWeather 배열의 모든 요소에 대해 콜백 함수를 실행하여 모든 요소가 true인 경우에만 true를 반환. 즉, 모든 필터 조건이 item.tag에 포함되어 있어야만 true가 반환.
      });
  ...
  return (
    <main className={`mw ${style.codiLog}`}>
      ...
      <button className={styleBtn.writeBtn} onClick={goLogWrite}>
        기록하기
      </button>
