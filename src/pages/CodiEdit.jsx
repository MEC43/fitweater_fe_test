...
const CodiEdit = () => {
  ...
  useEffect(() => {
    fetch(`${url}/codi/codiLogDetail/${codiLogId}`) //get요청 보냄
      .then((res) => res.json())
      .then((data) => {
        setOriginLog(data);
        // console.log('---선택 기록 data 전달 성공----', data);
        setCodiDate(
          `${data.codiDate.split('-')[0]}년 ${data.codiDate.split('-')[1]}월 ${
            data.codiDate.split('-')[2]
          }일`
        );
  ...
  const realSubmit = async () => {
    ...
    try {
      const response = await fetch(`${url}/codi/codiEdit/${codiLogId}`, {
        method: 'PUT',
        body: data,
      });
      if (response.ok) {
  ...
  return (
    <main className={`mw ${style.codiWrite}`}>
      ...
      <section className={style.codiWriteFrame}>
        ...
        <div className={style.weatherInfo}>
          ...
          <span className={`fontTitleS ${style.temp}`}>
            {originLog.maxTemp}°C/{originLog.minTemp}°C
          </span>
      ...
      <div className={style.bottomBtns}>
        <button className={`fontTitleM ${style.btnCancel}`} onClick={goBack}>
          취소
        </button>
        <button
